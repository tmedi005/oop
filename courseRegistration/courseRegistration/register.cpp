...
void processEnrollmentRequests(std::istream& ins, StudentList& students)
{
    ...
    while (getline(ins, studentName, ';')) {
        ...
        StudentList::iterator it = find(students.begin(), students.end(), Student(studentName));
        ...
        if (it != students.end()) {
            ...
            if (success) {
                cout << "(S) " << it->getName() << " enrolled in " << courseNumber << "\n";
            }
            else {
                cout << "(F) " << it->getName() << " not enrolled in " << courseNumber << "\n";
            }
        }
